...
export const buttonVariants = cva(
  "btn-press inline-flex items-center justify-center gap-2 font-semibold rounded-[var(--radius)] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--focus-ring)] focus-visible:ring-offset-2 disabled:opacity-[var(--opacity-disabled)] disabled:pointer-events-none select-none",
  {
    variants: {
      variant: {
        primary:
          "bg-[var(--brand)] text-[var(--ink-inverse)] shadow-[var(--shadow-sm)] hover:bg-[var(--brand-hover)]",
        secondary:
        ...
        danger:
          "bg-[var(--danger)] text-[var(--ink-inverse)] hover:bg-[var(--danger-hover)] shadow-[var(--shadow-sm)]",
        soft: "bg-[var(--brand-soft)] text-[var(--brand-hover)] hover:bg-[var(--brand-muted)]",
        success:
          "bg-[var(--success)] text-[var(--ink-inverse)] hover:opacity-90 shadow-[var(--shadow-sm)]",
      },
      size: {
